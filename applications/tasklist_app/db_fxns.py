...
def create_table():
    c.execute(
        "CREATE TABLE IF NOT EXISTS taskstable(task_doer TEXT,task_name TEXT,task_status TEXT,task_due_date DATE)"
    )


def add_data(task_doer, task_name, task_status, task_due_date):
    c.execute(
        "INSERT INTO taskstable(task_doer,task_name,task_status,task_due_date) VALUES (?,?,?,?)",
...
def edit_task_data(
    ...
    c.execute(
        "UPDATE taskstable SET task_doer =?,task_name =?,task_status=?,task_due_date=? WHERE task_doer=? and task_name=? and task_status=? and task_due_date=?",
        (
            new_task_doer,
            new_task_name,
            new_task_status,
